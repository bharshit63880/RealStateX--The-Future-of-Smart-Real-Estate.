...
export function WorkspacePage({ moduleKey }) {
  ...
  useEffect(()=>{let active=true;setData(null);apiClient(module.endpoint).then(payload=>{if(active){setData(payload.data);setError('');}}).catch(requestError=>{if(active)setError(requestError.message);});return()=>{active=false;};},[module.endpoint,refresh]);
