...
const MODULES={
  ...
  listings:{title:'Listing studio',intro:'Create, edit and submit properties through the verification lifecycle.',endpoint:'/properties/mine'},
  moderation:{title:'Listing review',intro:'Verify submitted inventory before it reaches the marketplace.',endpoint:'/moderation/listings'},
  users:{title:'User governance',intro:'Manage account access, roles and platform trust.',endpoint:'/admin/users'},
};
const LIVE_MODULES=new Set(['bookings','messages','offers','notifications','maintenance']);
const value=(item,key)=>{if(key==='bookings')return `${item.propertyId?.title||'Property visit'} · ${new Date(item.scheduledFor).toLocaleString()}`;if(key==='messages')return `${item.propertyId?.title||'Conversation'} · ${item.messages?.at(-1)?.body||'No messages'}`;if(key==='offers')return `${item.propertyId?.title||'Property'} · ${item.currency} ${Number(item.amount).toLocaleString('en-IN')}`;if(key==='documents')return `${item.name} · ${item.category}`;if(key==='notifications')return item.body;if(key==='projects')return `${item.city}, ${item.locality} · ${item.units?.length||0} units`;if(key==='leads')return `${item.email||item.phone||'No contact'} · score ${item.score}`;if(key==='leases')return `${item.propertyId?.title||'Property'} · ₹${Number(item.monthlyRent).toLocaleString('en-IN')}/month`;if(key==='maintenance')return `${item.priority} priority · ${item.propertyId?.title||'Property'}`;if(key==='listings'||key==='moderation')return `${item.locality}, ${item.city} · ₹${Number(item.price).toLocaleString('en-IN')}`;if(key==='users')return `${item.email} · ${item.role}`;return null;};

export function WorkspacePage({ moduleKey }) {
  const module=MODULES[moduleKey];const[data,setData]=useState(null);const[error,setError]=useState('');const[refresh,setRefresh]=useState(0);
  useEffect(()=>{let active=true;let timer;const load=async({silent=false}={})=>{if(!silent)setData(null);try{const payload=await apiClient(module.endpoint);if(active){setData(payload.data);setError('');}}catch(requestError){if(active)setError(requestError.message);}};load();if(LIVE_MODULES.has(moduleKey))timer=window.setInterval(()=>load({silent:true}),10000);return()=>{active=false;if(timer)window.clearInterval(timer);};},[module.endpoint,moduleKey,refresh]);
  const items=useMemo(()=>Array.isArray(data)?data:[],[data]);
  async function readAll(){try{await apiClient('/notifications/read-all',{method:'PATCH'});setRefresh(value=>value+1);}catch(requestError){setError(requestError.message);}}
  return <main className="workspace-page"><header><div><p className="arbor-label">Authenticated module</p><h1>{module.title}</h1><p>{module.intro}</p></div><div className="workspace-header-actions">{moduleKey==='notifications'&&items.some(item=>!item.readAt)&&<button type="button" onClick={readAll}>Mark all read</button>}<Link to={ROUTES.DASHBOARD}>← Dashboard</Link></div></header>
