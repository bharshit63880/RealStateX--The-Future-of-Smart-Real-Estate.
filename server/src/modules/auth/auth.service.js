...
export const login = async (input, context) => {
  const user = await User.findOne({ email: input.email.toLowerCase() }).select('+passwordHash +failedLoginAttempts +lockedUntil +securityVersion');
...
export const rotate = async (refreshToken, context) => {
  ...
  const user = await User.findById(payload.sub).select('+securityVersion'); if (!user || user.status !== 'ACTIVE') throw new UnauthorizedError();
...
export const revokeAll = (userId) => Promise.all([Session.updateMany({ userId, revokedAt: null }, { revokedAt: new Date() }),User.updateOne({_id:userId},{$inc:{securityVersion:1}})]);
export const changePassword = async (userId, currentPassword, newPassword) => {
  const user=await User.findById(userId).select('+passwordHash +securityVersion');
  if(!user||!await bcrypt.compare(currentPassword,user.passwordHash))throw new UnauthorizedError('Current password is incorrect');
  if(await bcrypt.compare(newPassword,user.passwordHash))throw new BadRequestError('New password must be different');
  user.passwordHash=await bcrypt.hash(newPassword,12);
  user.securityVersion+=1;
  await user.save();
  await Session.updateMany({userId:user.id,revokedAt:null},{revokedAt:new Date()});
  return user;
};
